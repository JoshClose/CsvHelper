...
/// <summary>
/// Used to replace characters in a <see cref="TextReader"/>.
/// </summary>
public class ReplaceTextReader : TextReader
{
	...
	/// <summary>
	/// Initializes a new instance using the given replacement.
	/// </summary>
	/// <param name="reader">The <see cref="TextReader"/>.</param>
	/// <param name="from">The replacement from text.</param>
	/// <param name="to">The replacement to text.</param>
	public ReplaceTextReader(TextReader reader, string from, string to) : this(reader, new Replacements { { from, to } }) { }

	/// <summary>
	/// Initializes a new instance using the given replacements.
	/// </summary>
	/// <param name="reader">The <see cref="TextReader"/>.</param>
	/// <param name="replacements">The replacements.</param>
	public ReplaceTextReader(TextReader reader, params (string, string)[] replacements) : this(reader, new Replacements(replacements)) { }
	...
	private void ValidateReplacements()
	{
		foreach (var replacement in replacements)
		{
			...
			{
				throw new ConfigurationException($"{nameof(Replacement)} {nameof(replacement.From)} cannot be empty.");
			}

			if (replacement.From.SequenceEqual(replacement.To))
			{
				throw new ConfigurationException($"{nameof(Replacement)} {nameof(replacement.From)} and {nameof(replacement.To)} cannot be the same.");
			}
		}

		if (replacements.GroupBy(r => new string(r.From)).Any(g => g.Count() > 1))
		{
			throw new ConfigurationException($"{nameof(Replacement)} {nameof(Replacement.From)} cannot be the same.");
		}

		if (replacements.Any(r1 => replacements.Any(r2 => !r1.Equals(r2) && new string(r2.From).Contains(new string(r1.From)))))
		{
			throw new ConfigurationException($"{nameof(Replacement)} {nameof(Replacement.From)} cannot contain any other {nameof(Replacement.From)}.");
		}
	}
