...
namespace CsvHelper
{
    ...
    public class CsvReader : ICsvReader
    {
        ...
        public virtual T GetField<T, TConverter>( int index ) where TConverter : ITypeConverter
        {
            ...
            var converter = ReflectionHelper.CreateInstance<TConverter>(Configuration != null ? Configuration.ContractResolver : null);
            return GetField<T>( index, converter );
        }
        ...
        public virtual T GetField<T, TConverter>( string name, int index ) where TConverter : ITypeConverter
        {
            ...
            var converter = ReflectionHelper.CreateInstance<TConverter>(Configuration?.ContractResolver);
            return GetField<T>( name, index, converter );
        }
        ...
        protected virtual void CreateFuncForObject( Type recordType )
        {
            ...
            if( Configuration != null && Configuration.ContractResolver != null )
            {
                if( Configuration.ContractResolver.CanCreate( recordType ) )
                {
                    var tmpType = Configuration.ContractResolver.Create( recordType, null );
                    recordType = tmpType.GetType();
                }
            }
            ...
            recordFuncs[origRecordType] = Expression.Lambda( funcType, body ).Compile();
        }
